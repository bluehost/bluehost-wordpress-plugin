	...
	if ( ! is_wp_error( $request ) ) {
		...
		if ( file_exists( $php_edge['location'] ) ) {
			...
			if ( ! is_wp_error( $php_check ) && wp_remote_retrieve_response_code( $php_check ) == '200' ) {
				$response = array(
					'status'  => 'success',
					'message' => 'PHP is updated to  ' . $updated_php_version,
				);
			} else {
				$response = array(
					'status'  => 'error',
					'message' => 'Site is incompatible with PHP Edge. Edge removed.',
				);
				mm_php_edge_remove();
				save_mod_rewrite_rules();
			}
		...
		$response = array(
			'status'  => 'error',
			'message' => 'Unable to enable PHP Edge.',
		);
	}
	return $response;
}
...
function mm_php_edge_remove( $type = null ) {
	...
	if ( file_exists( $file ) ) {
		if ( unlink( $file ) ) {
			$response = array(
				'status'  => 'success',
				'message' => 'PHP Edge successfully disabled.',
			);
		} else {
			$response = array(
				'status'  => 'error',
				'message' => 'Unable to remove PHP Edge.',
			);
		}
	} else {
		$response = array(
			'status'  => 'error',
			'message' => 'PHP Edge file does not exist.',
		);
	}
	return $response;
}
