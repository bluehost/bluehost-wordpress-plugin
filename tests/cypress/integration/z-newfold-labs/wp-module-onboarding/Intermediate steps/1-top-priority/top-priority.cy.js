...
describe( 'Top Priority Page', function () {
	before( () => {
       cy.visit(
			'wp-admin/?page=nfd-onboarding#/wp-setup/step/top-priority'
		);
	} );
    ...
    it( 'Click on different cards and move on to next page', () => {
		...
		arr.each( () => {
            ...
            cy.get('.nfd-card__body.nfd-selected-card')
                .find('.nfd-card__body_title').invoke('text').then(($ele) => {
                    if ($ele !== 'Selling'){
                        cy.log(previewCount);
                        cy.get('.components-button.navigation-buttons.navigation-buttons_next.is-primary').click();
                        cy.url().should('include', 'step/basic-info', { timeout: 30000 });
                        cy.go('back');
                        cy.get('.nfd-card__body.nfd-selected-card> .nfd-card__body_title').contains($ele);
                    }
                }); 

        });
        cy.get(className).eq(1).then(($el) => {
            cy.get( className )
            .eq( 1 )
            .should( 'be.visible' )
            .click();
            cy.get('.nfd-card__body.nfd-selected-card')
                .find('.nfd-card__body_title').invoke('text').then(($ele) => {
                    ...
                    cy.get('.components-button.navigation-buttons.navigation-buttons_next.is-primary').click();
                    cy.wait(10000);
                    cy.url().should('include', 'step/address', { timeout: 60000 });
                }); 
        });
    });
