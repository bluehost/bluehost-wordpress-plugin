...
describe('Theme Styles Menu and Preview', function () {
    ...
    it('Checks Active Link in the drawer is Theme Styles', () => {
        cy.get('.nfd-onboarding-drawer__panel-back').should('be.visible').should('have.text', 'Onboarding Menu');
    ...
    it('Check to make sure sidebar opens, content is in place and close sidebar with X', () => {
        ...
        cy.get('.nfd-onboarding-sidebar-learn-more__design-theme-styles-menu').should('be.visible');
    ...
    it('Check if Default Theme variations exists in Menu', () => {
        ...
        arr.each(() => {
            cy.get(className.concat('__title-bar')).eq(previewCount).scrollIntoView().should('be.visible');
            cy.get(className.concat('__live-preview-container')).eq(previewCount).scrollIntoView().should('be.visible');
            previewCount += 1;
        });
    });
    ...
    it('Check for selected Theme in preview when navigated back to Menu page', () => {
        ...
        cy.url().should('not.include', '#/wp-setup/step/design/theme-styles/preview');
        const className = ':nth-child(2) > .theme-styles-menu__list__item__title-bar';
        cy.get(className);
