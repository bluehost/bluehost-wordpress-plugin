...
export const CheckIllustrationPanel = (headingTag = '--illustration-panel') => {
    const heading = '.nfd-onboarding-sidebar-learn-more'.concat(headingTag);
    cy.get(heading.concat('__container')).should('be.visible');
    cy.get(heading.concat('__container > div')).should('be.visible').should('have.css', 'background-image');
}

export const CheckInfoPanel = ( infoCount = 1 ) => {
    cy.get('.nfd-onboarding-sidebar-learn-more--info-panel').should('be.visible');
    const infoTag = '.nfd-onboarding-sidebar-learn-more--heading-with-description';
    cy.get(infoTag.concat('__heading')).should('be.visible').should('have.length', infoCount);
    cy.get(infoTag.concat('__description')).should('be.visible').should('have.length', infoCount);
}

export const CheckHelpPanelLinks = (blueButton = false, whiteButtonText = 'Hire Our Full-Service Creative Studio', supportLinkText = 'Technical Support') => {
